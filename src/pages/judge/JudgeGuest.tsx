...
const JudgeGuest = () => {
  ...
  const handleAddOrUpdate = () => {
    ...
    if (editingUiId) {
      ...
      // UPDATED LIMIT TO 3
      if (guests.length >= 3) { toast.error("Maximum allocation (3) reached."); return; }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 p-8">
            ...
            <div className="flex flex-col items-center text-center space-y-6">
              ...
              <div className="space-y-3">
                ...
                <p className="text-slate-500 text-sm leading-relaxed font-medium px-2">
                  Kindly confirm whether these will be the only guests accompanying you.{" "}
      ...
      <div className="bg-[#1a3a32]/5 border border-[#1a3a32]/10 rounded-[2rem] p-8 flex flex-col md:flex-row gap-6 items-center">
        ...
        <div className="space-y-2">
          ...
          <p className="text-slate-600 text-sm leading-relaxed">
            ...
            <span className="font-bold text-slate-900">3 guests</span>. Ensure names match their official Identification Documents.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm">
        ...
        <div className="flex items-center gap-4 mt-8 pt-6 border-t border-slate-100">
          <button
            ...
            disabled={(guests.length >= 3 && !editingUiId) || busy}
                ...
                {guests.map((g, index) => (
                  <tr
                    ...
                    className={`border-b border-slate-50 transition-colors ${
                      editingUiId === g.uiId ? "bg-amber-50/60" : "hover:bg-slate-50/60"
                    }`}
                  >
                    ...
                    <td className="px-6 py-4 font-bold text-slate-800 text-xs">{g.name}</td>
                    <td className="px-6 py-4">
                      <span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full ${
                          g.type === "ADULT" ? "bg-[#1a3a32]/10 text-[#1a3a32]" : "bg-blue-50 text-blue-700"
                      }`}>
                        {g.type}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-xs font-semibold text-slate-500">{g.gender}</td>
                    <td className="px-6 py-4 text-xs font-mono text-slate-600">
                      {g.type === "ADULT" ? g.id_number : g.birth_cert_number}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          disabled={busy}
                          className="flex items-center gap-1.5 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-[#1a3a32] border border-[#1a3a32]/20 rounded-xl hover:bg-[#1a3a32]/5 transition-all"
                        >
                          <Pencil size={12} /> Edit
                        </button>
                        <button
                          ...
                          disabled={busy}
                          className="flex items-center gap-1.5 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-red-500 border border-red-100 rounded-xl hover:bg-red-50 transition-all"
                        >
      ...
      <div className="flex flex-col md:flex-row justify-end items-center gap-4 pt-8 border-t border-slate-100">
        ...
        >
          {status === "SUBMITTED" ? "Update Registration" : "Finalize Registry"} <ArrowRight size={14} />
        </button>
      </div>
    </div>
  );
...
/* =====================================================
    REUSABLE SUB-COMPONENTS (UNTOUCHED)
===================================================== */
