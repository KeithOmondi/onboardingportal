...
const SuperAdminNotice = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Prepare FormData for the backend Multer middleware
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      {/* Header & Feedback Messages (Remains Same) */}
                ...
                <tr key={notice.id} className="group hover:bg-stone-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleOpenModal(notice)} className="p-2 text-stone-400 hover:text-[#1a3a2a] rounded-lg transition-all"><Edit3 size={16} /></button>
                      <button onClick={() => handleDelete(notice.id)} className="p-2 text-stone-400 hover:text-red-600 rounded-lg transition-all"><Trash2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
  ...
  <select 
    ...
    onChange={(e) => setFormData({
      ...formData, 
      category: e.target.value as NoticeCategory // Use the imported type here
    })}
  >
