...
const SortIcon = ({ col, sortKey, sortDir }: { col: SortKey; sortKey: SortKey; sortDir: SortDir }) => {
  ...
  return sortDir === "asc" ? <ChevronUp size={13} className="inline ml-1 text-[#355E3B]" /> : <ChevronDown size={13} className="inline ml-1 text-[#355E3B]" />;
};
...
const SuperAdminGuests = () => {
  ...
  // Data Selectors
  const { allLists, loading: listLoading, expandedRegistry, expandLoading } = useSelector((state: RootState) => state.guests.admin);
  ...
  // ── Logic: Cross-Reference Users vs Registries ──────────────────────────────
    ...
    <div className="max-w-7xl mx-auto p-8 space-y-8 animate-in fade-in duration-700">
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="text-left">
              ...
              <p className="text-xs font-bold opacity-80 leading-none flex items-center gap-2">Choose Format <ChevronDown size={12} /></p>
            </div>
          </button>
          {showExportMenu && (
            <div className="absolute right-0 mt-3 w-56 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 p-2 overflow-hidden">
                <button onClick={() => handleBulkExport("pdf")} className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-slate-600 hover:bg-slate-50 rounded-xl transition-colors"><FileText size={16} className="text-red-500" /> PDF Document</button>
                <button onClick={() => handleBulkExport("excel")} className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-slate-600 hover:bg-slate-50 rounded-xl transition-colors"><FileSpreadsheet size={16} className="text-green-600" /> Excel Spreadsheet</button>
                <button onClick={() => handleBulkExport("word")} className="w-full flex items-center gap-3 px-4 py-3 text-xs font-bold text-slate-600 hover:bg-slate-50 rounded-xl transition-colors"><FileStack size={16} className="text-blue-600" /> Word Document</button>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[280px]">
          ...
          <input 
            type="text" placeholder="Search by judge name or ID..." value={search} onChange={(e) => setSearch(e.target.value)}
          ...
          {(["ALL", "SUBMITTED", "DRAFT", "NOT_STARTED"] as const).map((f) => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${
                filter === f ? "bg-[#1a3a32] text-white border-transparent" : "bg-white text-slate-500 border-slate-200"
              }`}
            >
                  ...
                  <React.Fragment key={row.user_id}>
                    <tr className={`hover:bg-slate-50/80 transition-all ${isExpanded ? "bg-slate-50/50" : ""}`}>
                      <td className="px-8 py-6">
                        <p className={`text-sm font-bold ${isZero ? "text-red-600" : "text-[#355E3B]"}`}>{row.judge_name}</p>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button 
                            ...
                            className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${
                              isExpanded ? "bg-[#355E3B] text-white shadow-lg" : "bg-white text-[#355E3B] hover:bg-slate-50 disabled:opacity-20"
                            }`}
                          >
                        ...
                        <td colSpan={5} className="px-8 py-8 animate-in slide-in-from-top-2 duration-300">
                          {expandLoading ? (
                             <div className="flex items-center justify-center gap-3 py-8 text-slate-400">
                               ...
                               <span className="text-[10px] font-black uppercase tracking-[0.2em]">Loading Authorized Guests...</span>
                             </div>
                          ) : expandedRegistry && (
