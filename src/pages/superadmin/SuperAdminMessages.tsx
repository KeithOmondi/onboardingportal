...
const SuperAdminMessages = () => {
  ...
  return (
    <div className="flex w-full overflow-hidden bg-white border-t border-[#d5e0cc] h-[calc(100vh-128px)] font-sans">
      {/* ── Sidebar ── */}
      <div className={`flex-col flex-shrink-0 bg-white border-r border-[#d5e0cc] w-full md:w-80 lg:w-96 ${showSidebar ? "flex" : "hidden"} md:flex`}>
        <div className="p-4 border-b border-[#d5e0cc]">
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-center gap-1.5 px-2 py-1 bg-[#f7faf5] rounded-full border border-[#eef5e8]">
              ...
              <span className="text-[9px] font-bold text-[#3a6644] uppercase">{connected ? "Online" : "Offline"}</span>
            </div>
          </div>

          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8aaa92]" size={14} />
            <input
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder={activeTab === "direct" ? "Search contacts..." : "Search users to include..."}
          ...
          {(["direct", "broadcast"] as const).map((t) => (
            <button
              ...
              className={`flex-1 py-3 text-[11px] font-black uppercase tracking-widest transition-all ${
                activeTab === t ? "text-[#3a6644] border-b-2 border-[#3a6644]" : "text-[#8aaa92]"
              }`}
            >
             ...
             <div className="space-y-1">
               {filteredRecipients.map(r => (
                 <RecipientRow 
                    ...
                    onClick={() => { selectRecipient(r); setShowSidebar(false); }}
                 />
               ))}
             </div>
          ) : (
            <div className="space-y-6 p-2">
              <div className="space-y-3">
                ...
                <label className="flex items-center gap-3 text-xs font-bold text-[#1a3a2a] cursor-pointer">
                  <input type="radio" checked={broadcastType === "broadcast"} onChange={() => setBroadcastType("broadcast")} className="accent-[#3a6644]" />
                  Global Broadcast (All)
                </label>
                <label className="flex items-center gap-3 text-xs font-bold text-[#1a3a2a] cursor-pointer">
                  <input type="radio" checked={broadcastType === "group"} onChange={() => setBroadcastType("group")} className="accent-[#3a6644]" />
                  Role-Based Group
                </label>
              </div>
              ...
              {broadcastType === "group" && (
                <div className="pl-6 space-y-2 border-l-2 border-[#f0f5ec]">
                  {ROLES.map(role => (
                    <label key={role} className="flex items-center gap-3 text-xs text-[#1a3a2a] cursor-pointer">
                      <input 
                        type="checkbox" 
              ...
              <div className="space-y-3">
                ...
                <div className="border border-[#eef5e8] rounded-xl overflow-hidden max-h-60 overflow-y-auto">
                  {filteredRecipients.map(r => (
                    <div key={r.id} onClick={() => toggleUser(r.id)} className="flex items-center gap-3 p-3 border-b border-[#eef5e8] hover:bg-[#f7faf5] cursor-pointer">
                      {selectedUserIds.includes(r.id) ? <CheckCircle2 size={16} className="text-[#3a6644]" /> : <Circle size={16} className="text-[#cddcc5]" />}
                      <span className="text-[11px] font-bold text-[#1a3a2a]">{r.full_name}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* ── Main Area ── */}
      <div className={`flex-1 flex flex-col bg-[#f7faf5] ${!showSidebar ? "flex" : "hidden"} md:flex`}>
        <div className="h-[65px] px-6 flex items-center justify-between border-b border-[#d5e0cc] bg-white">
          <div className="flex items-center gap-4">
            <button onClick={() => setShowSidebar(true)} className="md:hidden"><ArrowLeft size={20} /></button>
            <div>
              <h3 className="text-sm font-black text-[#1a3a2a]">
                {activeTab === "broadcast" ? "Administrative Relay History" : selectedRecipient?.full_name || "Select a Recipient"}
              </h3>
              <p className="text-[10px] font-bold text-[#8aaa92] uppercase tracking-widest">
                {activeTab === "broadcast" ? "One-Way Communication" : "Two-Way Direct Chat"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4 custom-scrollbar">
          {((activeTab === "direct" && loadingHistory) || (activeTab === "broadcast" && loadingBroadcasts)) && (
             <div className="flex flex-col items-center justify-center py-10 text-[#3a6644]">
               <Loader2 className="animate-spin mb-2" size={24} />
               <span className="text-[10px] font-black uppercase">Syncing Relay History...</span>
             </div>
          )}

          {(activeTab === "direct" ? conversationMessages : broadcastMessages).map((msg, i) => {
            const isMine = msg.sender_id === currentUser?.id;
            
            // In the one-way relay, we only show outgoing admin communications
            if (activeTab === "broadcast" && !isMine) return null;

            return (
              <div key={i} className={`flex flex-col ${isMine ? "items-end" : "items-start"}`}>
                <div className={`px-4 py-2.5 rounded-2xl text-[13px] shadow-sm max-w-[70%] ${
                  isMine ? "bg-[#3a6644] text-white rounded-tr-none" : "bg-white border border-[#d5e0cc] text-[#1a3a2a] rounded-tl-none"
                }`}>
                  {activeTab === "broadcast" && (
        ...
        {/* Dynamic Input Bar: Functions as Direct Chat OR Broadcast Relay */}
            ...
            <div className="flex gap-3 max-w-4xl mx-auto">
              <input
                ...
                onKeyDown={(e) => e.key === "Enter" && (
                  activeTab === "direct" 
                    ? (selectedRecipient && (sendToUser(input, selectedRecipient.id), setInput("")))
                    : handleSendBroadcast()
                )}
                placeholder={activeTab === "broadcast" ? "Type global relay message..." : "Type message..."}
              ...
              <button
                onClick={activeTab === "direct" 
                  ? () => { if(selectedRecipient) { sendToUser(input, selectedRecipient.id); setInput(""); } } 
                  : handleSendBroadcast
                }
...
const RecipientRow = ({ recipient, isSelected, onClick }: { recipient: Recipient; isSelected: boolean; onClick: () => void }) => (
  <button 
    onClick={onClick}
    className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all border-l-4 ${
      isSelected ? "bg-[#f0f5ec] border-[#3a6644]" : "border-transparent hover:bg-gray-50"
    }`}
  >
    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-[10px] font-black border flex-shrink-0 ${AVATAR_COLOR[recipient.role]}`}>
      {initials(recipient.full_name)}
    </div>
    <div className="flex-1 text-left min-w-0">
      <div className="flex justify-between items-center">
        <p className="text-xs font-bold text-[#1a3a2a] truncate">{recipient.full_name}</p>
        {(recipient.unreadCount ?? 0) > 0 && (
          <span className="bg-red-500 text-white text-[9px] font-black rounded-full min-w-[18px] h-[18px] flex items-center justify-center animate-bounce">
            {recipient.unreadCount}
          </span>
        )}
      </div>
      <p className="text-[10px] text-[#8aaa92] uppercase tracking-tighter font-semibold">{recipient.role}</p>
    </div>
  </button>
);
