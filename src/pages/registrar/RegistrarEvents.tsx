...
// Utility for status-based styling
const getStatusStyles = (status: EventStatus) => {
  switch (status) {
    ...
    default: return { color: "text-[#c2a336]", bg: "bg-amber-50" };
  }
};

const RegistrarEvents = () => {
  const dispatch = useAppDispatch();
  const { events, loading, error, currentFilter } = useAppSelector((state) => state.events);
  ...
  const nextSession = events.length > 0 ? events[0] : null;
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold font-serif text-[#1a3a32] uppercase tracking-tight">
            2026 HIGH COURT JUDGES EVENTS
          </h1>
          <p className="text-[10px] font-black text-[#c2a336] uppercase tracking-[0.2em] mt-1">
        ...
        <div className="space-y-4">
          <div className="bg-white border border-stone-200 p-5 rounded-2xl shadow-sm">
            <h3 className="text-xs font-black uppercase tracking-widest text-stone-400 mb-4 flex items-center gap-2">
              <ClipboardList size={14} /> Events Summary
            </h3>
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-stone-600">Total Listed</span>
                <span className="text-lg font-bold text-[#1a3a32]">{events.length}</span>
              </div>
              <div className="h-px bg-stone-100" />
              <p className="text-[11px] text-stone-400 italic">
                Viewing {currentFilter.toLowerCase()} sessions.
              </p>
            </div>
          </div>

          <div className="bg-[#1a3a32] text-white p-6 rounded-2xl shadow-lg relative overflow-hidden">
            <div className="relative z-10">
              <CalendarIcon size={24} className="text-[#c2a336] mb-4" />
              <p className="text-sm font-medium opacity-80">Next Event</p>
              {nextSession ? (
                <>
                  <h4 className="text-lg font-bold font-serif mt-1">
                    {new Date(nextSession.start_time).toLocaleDateString('en-KE', { month: 'long', day: 'numeric' })}
                  </h4>
                  <p className="text-xs mt-4 font-black uppercase tracking-widest text-[#c2a336]">
        ...
        <div className="lg:col-span-3 space-y-6">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 text-stone-400">
              ...
              <p className="text-sm font-medium">Syncing event data...</p>
            </div>
          ) : error ? (
            ...
            <div className="text-center py-20 bg-stone-50 rounded-2xl border border-dashed border-stone-200">
              <p className="text-stone-400 font-medium">No sessions found for this category.</p>
            </div>
          ) : (
            events.map((event) => {
              const eventDate = new Date(event.start_time);
              const statusStyle = getStatusStyles(currentFilter);
              ...
              return (
                <div key={event.id} className="group relative flex gap-6">
                  ...
                  <div className="hidden sm:flex flex-col items-center min-w-[60px] pt-2">
                    ...
                    <span className="text-2xl font-bold text-stone-800">{eventDate.getDate()}</span>
                    <div className="w-px h-full bg-stone-100 mt-2 group-last:bg-transparent" />
                  </div>

                  {/* Event Card */}
                  <div className="flex-1 bg-white border border-stone-200 rounded-2xl overflow-hidden hover:shadow-md transition-all duration-300">
                    <div className="p-6">
                      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                        <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${statusStyle.bg} ${statusStyle.color}`}>
                          <Clock size={12} />
                          {currentFilter === "ALL" ? "ORHC Event" : currentFilter}
                        </div>
                      ...
                      <h3 className="text-xl font-bold font-serif text-[#1a3a32] mb-2 group-hover:text-[#c2a336] transition-colors">
                        {event.title}
                      </h3>
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-stone-50">
                        <div className="flex items-center gap-4 text-stone-500">
                          <div className="flex items-center gap-1.5">
                            <Clock size={14} className="text-[#c2a336]" />
                        ...
                        <button className="flex items-center gap-1 text-xs font-black uppercase tracking-widest text-[#1a3a32] hover:text-[#c2a336] transition-colors">
...
export default RegistrarEvents;