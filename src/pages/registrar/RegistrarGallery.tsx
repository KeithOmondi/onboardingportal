...
import { 
  ...
  Film, Image as ImageIcon, LayoutGrid, Loader2
} from "lucide-react";
...
const RegistrarGallery = () => {
  ...
  useEffect(() => {
    // Fetch all items; we'll handle the category filtering on the client 
    // for a smoother UI, or you can pass activeCategory if your API supports it.
  ...
  // Client-side filtering for high-speed interaction
    ...
    <div className="flex flex-col h-full bg-[#F8FAFC] font-sans overflow-hidden">
                ...
                >
                  <div className="aspect-[4/3] relative bg-slate-900 overflow-hidden">
                    ...
                    <div className="absolute top-3 right-3">
                      <div className="bg-black/60 backdrop-blur-md px-2 py-1 rounded-md text-white text-[10px] flex items-center gap-1 uppercase font-black tracking-tighter">
                        {item.file_type === MediaType.VIDEO ? <Film size={10} /> : <ImageIcon size={10} />}
                        {item.file_type}
                      </div>
                    </div>
                    ...
                    <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                      <div className="flex-1 min-w-0">
                          <h3 className="text-white font-bold text-sm mt-1 truncate pr-2 uppercase">{item.title}</h3>
                          <div className="flex items-center gap-1 text-white/50 text-[10px] mt-0.5 uppercase font-bold tracking-widest">
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
          ...
          <div className="relative w-full max-w-5xl bg-[#1a3a32] rounded-[2rem] overflow-hidden shadow-2xl border border-white/10 flex flex-col md:flex-row animate-in zoom-in-95 duration-300 max-h-[90vh]">
             {/* Media Container */}
             <div className="flex-1 bg-black flex items-center justify-center overflow-hidden">
                {selectedItem.file_type === MediaType.VIDEO ? (
                  <video src={selectedItem.file_url} controls className="max-w-full max-h-full" />
                ) : (
                  <img src={selectedItem.file_url} alt={selectedItem.title} className="max-w-full max-h-full object-contain" />
                )}
             </div>
             ...
             <div className="w-full md:w-80 p-8 text-white flex flex-col">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <span className="text-[10px] font-black bg-[#c2a336] text-[#1a3a32] px-2 py-0.5 rounded tracking-widest uppercase">
                      Official Record #{selectedItem.id}
                    </span>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto">
                  <p className="text-slate-300 text-xs mb-8 bg-white/5 p-4 rounded-xl border border-white/5 italic">
                    {selectedItem.description || "No archival description provided for this asset."}
                  </p>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4 p-3 bg-white/5 rounded-xl">
                      ...
                      <div>
                        <p className="text-[8px] text-white/30 uppercase font-black">Archive Date</p>
                        <p className="text-xs font-bold">
                          {new Date(selectedItem.created_at).toLocaleDateString("en-KE", { dateStyle: 'long' })}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 p-3 bg-white/5 rounded-xl">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold uppercase">{selectedItem.file_type}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-8">
                  <a 
                    ...
                    target="_blank" 
                    rel="noreferrer"
                    className="block w-full text-center bg-[#c2a336] text-[#1a3a32] py-3 rounded-xl font-black text-xs hover:bg-[#d4b54d] transition-all"
                  >
