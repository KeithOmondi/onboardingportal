import { LayoutDashboard, LogOut, X, Users, Info, Camera, Megaphone, CalendarDays } from "lucide-react";
import { Link, useLocation } from "react-router-dom";
import { useDispatch } from "react-redux";
import { logout } from "../../redux/slices/authSlice";
// Import your AppDispatch type from your store definition
...
const RegistrarSidebar = ({ isOpen, onClose }: RegistrarSidebarProps) => {
  ...
  // Use the typed dispatch to allow Thunk actions like logout()
  const dispatch = useDispatch<AppDispatch>();

  const menuItems = [
    { icon: <LayoutDashboard size={20} />, label: "Dashboard", path: "/registrar/dashboard" },
    { icon: <Info size={20} />, label: "High Court Information", path: "/registrar/info" },
    { icon: <Megaphone size={20} />, label: "Notice Board", path: "/registrar/notices" },
    { icon: <CalendarDays size={20} />, label: "Events", path: "/registrar/events" },
    { icon: <Camera size={20} />, label: "Gallery", path: "/registrar/gallery" },
  ];
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-[#C5A059]/15 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#C5A059] rounded-xl flex items-center justify-center shadow-lg shadow-[#C5A059]/20 flex-shrink-0">
              <Users className="text-[#1a3320]" size={22} />
            </div>
            <div>
              <h1 className="text-white font-serif font-bold tracking-tight">ORHC</h1>
              <p className="text-[10px] font-serif uppercase text-[#C5A059] font-black tracking-widest">
                Registrar Portal
              </p>
            </div>
          </div>
        ...
        <div className="px-4 py-5 border-t border-[#C5A059]/15">
          ...
          <button 
            ...
            className="flex items-center gap-4 px-4 py-3 w-full text-white/40 hover:text-red-400 transition-colors rounded-xl hover:bg-white/5 group"
          >
            <LogOut size={20} />
