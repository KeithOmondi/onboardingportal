...
import { Bell, Search, UserCircle, Menu, LogOut, User } from "lucide-react";
...
const RegistrarHeader = ({ onMenuToggle }: RegistrarHeaderProps) => {
  ...
  return (
    <header className="h-16 lg:h-20 bg-[#355E3B] px-4 sm:px-6 lg:px-8 flex items-center justify-between sticky top-0 z-50 gap-3 shadow-lg shadow-[#355E3B]/30">
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <button onClick={onMenuToggle} className="lg:hidden p-2 text-white/70 hover:text-white hover:bg-white/10 rounded-xl transition-colors">
          <Menu size={22} />
        </button>
        ...
        <div className="relative w-full max-w-xs sm:max-w-sm lg:max-w-md hidden sm:block">
          ...
          <input
            type="text"
            placeholder="Search registrations, notices..."
            className="w-full bg-white/10 border border-white/10 rounded-2xl py-2.5 pl-11 pr-4 text-sm text-white placeholder:text-white/40 focus:ring-2 focus:ring-[#C5A059]/40 outline-none transition-all"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 sm:gap-5 flex-shrink-0">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-2xl py-2 border border-gray-100 animate-in fade-in zoom-in duration-150 origin-top-right">
              ...
              <button onClick={() => dispatch(logout())} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-red-600 hover:bg-red-50 transition-colors">
